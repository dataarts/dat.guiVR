...
import { Group, BoxGeometry, Matrix4, Mesh } from 'three';
...
export default function createTextLabel( textCreator, str, width = 0.4, depth = 0.029, fgColor = 0xffffff, bgColor = Colors.DEFAULT_BACK, scale = 1.0 ){
  ...
  const internalPositioning = new Group();
  ...
  const labelBackGeometry = new BoxGeometry( totalWidth, totalHeight, depth, 1, 1, 1 );
  labelBackGeometry.applyMatrix( new Matrix4().makeTranslation( totalWidth * 0.5 - margin, 0, 0 ) );

  const labelBackMesh = new Mesh( labelBackGeometry, SharedMaterials.PANEL );
